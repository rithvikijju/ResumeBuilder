...
${list}`;
  ...
  return [
    ...
    "",
    "2. ONE-PAGE LIMIT WITH MAXIMUM CONTENT:",
    "   - The resume MUST fit on a single page (8.5\" x 11\"), but include AS MUCH relevant information as possible.",
    "   - Include ALL relevant bullets from experiences and projects - do not limit to 3-4. Use compact formatting to fit more content.",
    "   - Prioritize by relevance to the job, but include as many bullets as can fit on one page.",
    "   - Use tight spacing and compact formatting to maximize content while maintaining readability.",
    "   - Keep summary to 2-3 sentences maximum if included.",
    "",
    "3. NO TITLE IN RESUME CONTENT:",
    "   - DO NOT include a resume title, heading, or name in the resume content itself.",
    "   - The resume content should start directly with sections (Education, Experience, Projects, etc.).",
    "   - Only include the header information (name, contact) if using structured format with header field.",
    "",
    `4. TEMPLATE STYLING: ${template.name} (${template.category})`,
    `   - Follow the template's section order EXACTLY: ${template.layout.sectionOrder.join(", ")}`,
    `   - ${template.layout.showSummary ? "Include" : "Do NOT include"} a professional summary.`,
    `   - Format skills as: ${template.layout.skillsFormat}`,
    `   - Match the professional styling and formatting conventions of this template category.`,
...
${JSON.stringify(
    ...
    "- The resume MUST fit on one page, but include AS MUCH relevant content as possible using compact formatting.",
    ...
    "- DO NOT include a title or heading like 'Resume' or the job title in the resume content - start directly with sections.",
    "- Include all relevant experiences, projects, education, and skills that fit on one page with proper formatting.",
  ].join("\n");
}
